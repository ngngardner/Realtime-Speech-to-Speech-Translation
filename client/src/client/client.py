...
class AudioSocketClient:
    ...
    def __init__(self) -> None:
        ...
        if input(" Is this correct?\n y/[n]: ") != "y":
            self.input_device_index = int(
                input("Type the index of the physical microphone: "),
            )
            self.output_device_index = int(
                input("Type the index of the output microphone: "),
            )
    ...
    def __volume_print_worker__(self):
        ...
        while True:
            if (
                abs(last_volume_input - self.volume_input) > 0.1
                or abs(last_volume_output - self.volume_output) > 0.1
            ):
            ...
            if (
                self.time_last_sent
                and time.time() - self.time_last_sent > self.PHRASE_TIME_LIMIT
            ):
                self.volume_input = 0
            time.sleep(0.1)
